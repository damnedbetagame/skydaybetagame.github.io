...
$data = "kullaniciadi=hesapadi&sifre=hesapsifre&benihatirla=on";
...
if (isset($_GET["tc"]))
{
   ...
   $url = "https://www.twikschecker.us/inc/api/ailesorgu";
...
$headers = array(
   ...
   "sec-fetch-mode: cors",
   "sec-fetch-site: same-origin",
   "twiksauthenticationtoken: ".$json->token."",
   "user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.131 Safari/537.36",
   "x-requested-with: XMLHttpRequest",
);
curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
...
$aile = json_decode($resp, true);

$sonuc = json_encode($aile['aileBilgisi']);
